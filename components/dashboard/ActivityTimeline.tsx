...
export default function ActivityTimeline({ payPoints }: ActivityTimelineProps) {
  ...
  if (recentPoints.length === 0) {
    return (
      <div className="p-6">
        <h3 className="mb-4 text-base font-bold text-[var(--text-main)]">Recent Activity</h3>
          ...
          return (
            <div key={point.id || `${point.year}-${point.pay}`} className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1 pb-2">
                ...
                <p className="text-xs text-[var(--text-muted)]">{formatRelativeTime(point.year)}</p>
                <div className="mt-1 flex items-center gap-2">
                  <span className="rounded-md bg-green-100 px-2 py-0.5 text-xs font-bold text-green-700">
                    + {point.pay.toLocaleString('nb-NO')} pts
                  </span>
                </div>
              </div>
            </div>
